  ...
  try {
    ...
    const [{ data: me }, { data: posts }, { data: photos }, { data: users }, { data: reactions }] =
      await Promise.all([
        ...
        supabaseAdmin.from("posts").select("id,user_id,type,caption,created_at").order("created_at", { ascending: false }).limit(30),
        supabaseAdmin.from("photos").select("post_id,kind,url"),
        supabaseAdmin.from("users").select("id,name,avatar_url"),
        supabaseAdmin.from("reactions").select("post_id,reaction_type"),
      ]);
    ...
    for (const r of reactions ?? []) {
      const current = reactionMap.get(r.post_id) ?? { like: 0, connect: 0, star: 0 };
      if (r.reaction_type === "like") current.like += 1;
      if (r.reaction_type === "connect") current.connect += 1;
      if (r.reaction_type === "star") current.star += 1;
      reactionMap.set(r.post_id, current);
    }
